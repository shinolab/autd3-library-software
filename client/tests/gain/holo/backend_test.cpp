...
TYPED_TEST(BackendTest, make_complex) {
  auto r = this->_backend->allocate_matrix("r", 2, 1);
  auto i = this->_backend->allocate_matrix("i", 2, 1);

  r->copy_from({0, 1});
  i->copy_from({2, 3});

  auto a = this->_backend->allocate_matrix_c("a", 2, 1);

  this->_backend->make_complex(r, i, a);
...
TYPED_TEST(BackendTest, scale) {
  auto a = this->_backend->allocate_matrix_c("a", 4, 1);
  a->copy_from({complex(0, 1), complex(2, 3), complex(4, 5), complex(6, 7)});

  this->_backend->scale(a, complex(1, 1));
...
TYPED_TEST(BackendTest, hadamard_product) {
  auto a = this->_backend->allocate_matrix_c("a", 2, 2);
  auto b = this->_backend->allocate_matrix_c("b", 2, 2);
  a->copy_from({complex(0, 1), complex(2, 3), complex(4, 5), complex(6, 7)});
  b->copy_from({complex(8, 9), complex(10, 11), complex(12, 13), complex(14, 15)});

  auto c = this->_backend->allocate_matrix_c("c", 2, 2);
...
TYPED_TEST(BackendTest, real) {
  ...
  auto b = this->_backend->allocate_matrix("b", 2, 2);
...
TYPED_TEST(BackendTest, arg) {
  ...
  auto b = this->_backend->allocate_matrix_c("b", 2, 1);

  this->_backend->arg(a, b);
...
TYPED_TEST(BackendTest, pseudo_inverse_svd_c) {
  ...
  auto b = this->_backend->allocate_matrix_c("b", n, n);
  this->_backend->pseudo_inverse_svd(a, 1e-4, b);

  auto c = this->_backend->allocate_matrix_c("c", n, n);
  this->_backend->matrix_mul(TRANSPOSE::NO_TRANS, TRANSPOSE::NO_TRANS, ONE, a, b, ZERO, c);
  c->copy_to_host();
  for (Eigen::Index i = 0; i < n; i++)
    for (Eigen::Index j = 0; j < n; j++) {
      if (i == j)
...
TYPED_TEST(BackendTest, matrix_add) {
  auto a = this->_backend->allocate_matrix("a", 2, 2);
  a->copy_from({0, 2, 1, 3});

  auto b = this->_backend->allocate_matrix("b", 2, 2);
  b->fill(0.0);

  this->_backend->matrix_add(0.0, a, b);
  b->copy_to_host();
  ...
  this->_backend->matrix_add(2.0, a, b);
  b->copy_to_host();
  ASSERT_NEAR(b->data(0, 0), 0.0, 1e-6);
...
TYPED_TEST(BackendTest, matrix_mul_c) {
  ...
  this->_backend->matrix_mul(TRANSPOSE::CONJ_TRANS, TRANSPOSE::TRANS, ZERO, a, b, ONE, c);
  c->copy_to_host();
  ASSERT_NEAR_COMPLEX(c->data(0, 0), complex(-24, 70), 1e-6);
  ASSERT_NEAR_COMPLEX(c->data(0, 1), complex(-28, 82), 1e-6);
  ASSERT_NEAR_COMPLEX(c->data(1, 0), complex(-32, 238), 1e-6);
...
TYPED_TEST(BackendTest, matrix_mul) {
  ...
  auto c = this->_backend->allocate_matrix("c", 2, 2);

  this->_backend->matrix_mul(TRANSPOSE::NO_TRANS, TRANSPOSE::TRANS, 1.0, a, b, 0.0, c);
  c->copy_to_host();
  ...
  this->_backend->matrix_mul(TRANSPOSE::TRANS, TRANSPOSE::NO_TRANS, 1.0, a, b, 1, c);
  c->copy_to_host();
...
TYPED_TEST(BackendTest, solve_ch) {
  ...
  auto tmp = this->_backend->allocate_matrix_c("tmp", n, n);
  tmp->copy_from(random_vector_complex(n * n));

  auto a = this->_backend->allocate_matrix_c("a", n, n);
  this->_backend->matrix_mul(TRANSPOSE::NO_TRANS, TRANSPOSE::CONJ_TRANS, ONE, tmp, tmp, ZERO, a);
  auto x_expected = random_vector_complex(n);

  auto x = this->_backend->allocate_matrix_c("x", n, 1);
  x->copy_from(x_expected);

  auto b = this->_backend->allocate_matrix_c("b", n, 1);
  this->_backend->matrix_mul(TRANSPOSE::NO_TRANS, TRANSPOSE::NO_TRANS, ONE, a, x, ZERO, b);

  this->_backend->solve_ch(a, b);
...
TYPED_TEST(BackendTest, solve_g) {
  ...
  auto tmp = this->_backend->allocate_matrix("tmp", n, n);
  tmp->copy_from(random_vector(n * n));

  auto a = this->_backend->allocate_matrix("a", n, n);
  this->_backend->matrix_mul(TRANSPOSE::NO_TRANS, TRANSPOSE::TRANS, 1.0, tmp, tmp, 0.0, a);
  auto x_expected = random_vector(n);

  auto x = this->_backend->allocate_matrix("x", n, 1);
  x->copy_from(x_expected);

  auto b = this->_backend->allocate_matrix("b", n, 1);
  this->_backend->matrix_mul(TRANSPOSE::NO_TRANS, TRANSPOSE::NO_TRANS, 1.0, a, x, 0.0, b);

  auto xs = this->_backend->allocate_matrix("xs", n, 1);
  this->_backend->solve_g(a, b, xs);
...
TYPED_TEST(BackendTest, dot) {
  ...
  auto a = this->_backend->allocate_matrix("a", n, 1);
  a->copy_from(a_vals);
  auto b = this->_backend->allocate_matrix("b", n, 1);
  b->copy_from(b_vals);
...
TYPED_TEST(BackendTest, dot_c) {
  ...
  auto a = this->_backend->allocate_matrix_c("a", n, 1);
  a->copy_from(a_vals);
  auto b = this->_backend->allocate_matrix_c("b", n, 1);
  b->copy_from(b_vals);
...
TYPED_TEST(BackendTest, max_coefficient) {
  ...
  auto v = this->_backend->allocate_matrix("v", n, 1);
  v->copy_from(vals);
...
TYPED_TEST(BackendTest, concat_row) {
  auto a = this->_backend->allocate_matrix_c("a", 1, 2);
  auto b = this->_backend->allocate_matrix_c("b", 1, 2);
  a->copy_from({complex(0, 1), complex(2, 3)});
  b->copy_from({complex(8, 9), complex(10, 11)});

  auto c = this->_backend->concat_row(a, b);
...
TYPED_TEST(BackendTest, concat_col) {
  ...
  auto c = this->_backend->concat_col(a, b);
