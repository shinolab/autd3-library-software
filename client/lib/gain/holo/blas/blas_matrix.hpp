...
inline void BLASMatrix<complex>::pseudo_inverse_svd(const std::shared_ptr<EigenMatrix<complex>>& matrix, const double alpha,
                                                    const std::shared_ptr<EigenMatrix<complex>>& u, const std::shared_ptr<EigenMatrix<complex>>& s,
                                                    const std::shared_ptr<EigenMatrix<complex>>& vt,
...
inline void BLASMatrix<double>::pseudo_inverse_svd(const std::shared_ptr<EigenMatrix<double>>& matrix, const double alpha,
...
inline void BLASMatrix<complex>::max_eigen_vector(const std::shared_ptr<EigenMatrix<complex>>& ev) {
...
inline void BLASMatrix<double>::max_eigen_vector(const std::shared_ptr<EigenMatrix<double>>& ev) {}

inline void BLASMatrix<double>::add(const double alpha, const std::shared_ptr<EigenMatrix<double>>& a) {
  AUTD_AXPY(static_cast<int>(a->data.size()), alpha, a->data.data(), 1, data.data(), 1);
}
inline void BLASMatrix<complex>::add(const complex alpha, const std::shared_ptr<EigenMatrix<complex>>& a) {
  AUTD_AXPYC(static_cast<int>(a->data.size()), &alpha, a->data.data(), 1, data.data(), 1);
}
template <>
inline void BLASMatrix<double>::mul(const TRANSPOSE trans_a, const TRANSPOSE trans_b, const double alpha,
...
template <>
inline void BLASMatrix<complex>::mul(TRANSPOSE trans_a, TRANSPOSE trans_b, const complex alpha, const std::shared_ptr<const EigenMatrix<complex>>& a,
...
inline void BLASMatrix<double>::solve(const std::shared_ptr<EigenMatrix<double>>& b) {
...
inline void BLASMatrix<complex>::solve(const std::shared_ptr<EigenMatrix<complex>>& b) {
  const auto n = static_cast<int>(data.cols());
  const auto lda = static_cast<int>(data.rows());
  const auto ldb = static_cast<int>(b->data.size());
...
inline double BLASMatrix<double>::dot(const std::shared_ptr<const EigenMatrix<double>>& a) {
  return AUTD_DOT(static_cast<int>(a->data.size()), data.data(), 1, a->data.data(), 1);
}
inline complex BLASMatrix<complex>::dot(const std::shared_ptr<const EigenMatrix<complex>>& a) {
  complex d;
...
inline void BLASMatrix<double>::copy_from(const double* v, const size_t n) { AUTD_CPY(static_cast<int>(n), v, 1, data.data(), 1); }
inline void BLASMatrix<complex>::copy_from(const complex* v, const size_t n) { AUTD_CPYC(static_cast<int>(n), v, 1, data.data(), 1); }
