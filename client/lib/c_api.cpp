...
int AUTDSetFocalPointLM(AUTDControllerHandle handle, float x, float y, float z, float lmamp_x, float lmamp_y, float lmamp_z, float freq,
                        uint8_t amp) {
  auto *cnt = static_cast<Controller *>(handle);
...
int AUTDSetBesselBeamSine(AUTDControllerHandle handle, float x, float y, float z, float vec_x, float vec_y, float vec_z, float theta_z, float freq,
                          float amp, float offset) {
  auto *cnt = static_cast<Controller *>(handle);
  auto g = BesselBeamGain::Create(Vector3f(x, y, z), Vector3f(vec_x, vec_y, vec_z), theta_z);
  cnt->AppendGainSync(g);
  auto m = SineModulation::Create(freq, amp, offset);
