...
constexpr double DIR_COEFF_A_D[] = {1.0, 1.0, 1.0, 0.891250938, 0.707945784, 0.501187234, 0.354813389, 0.251188643, 0.199526231};
constexpr double DIR_COEFF_B_D[] = {
    0., 0., -0.00459648054721, -0.0155520765675, -0.0208114779827, -0.0182211227016, -0.0122437497109, -0.00780345575475, -0.00312857467007};
constexpr double DIR_COEFF_C_D[] = {
...
constexpr double DIR_COEFF_D_D[] = {
...
constexpr float DIR_COEFF_A_F[] = {1.0f, 1.0f, 1.0f, 0.891250938f, 0.707945784f, 0.501187234f, 0.354813389f, 0.251188643f, 0.199526231f};
constexpr float DIR_COEFF_B_F[] = {
...
constexpr float DIR_COEFF_C_F[] = {0.f,
                                   0.f,
                                   -0.000787968093807f,
                                   -0.000307591508224f,
                                   -0.000218348633296f,
                                   ...
                                   0.000323676257958f,
                                   0.000143850511f};
constexpr float DIR_COEFF_D_F[] = {0.f,
                                   0.f,
                                   1.60125528528e-05f,
                                   2.9747624976e-06f,
                                   2.31910931569e-05f,
...
static inline float directivityT4010A1(float theta_deg) {
  ...
  if (i == 0) return 1;
  const auto a = DIR_COEFF_A_F[i - 1];
  const auto b = DIR_COEFF_B_F[i - 1];
  const auto c = DIR_COEFF_C_F[i - 1];
  const auto d = DIR_COEFF_D_F[i - 1];
  const auto x = theta_deg - static_cast<float>(i - 1) * 10;
  return a + b * x + c * x * x + d * x * x * x;
}

static inline double directivityT4010A1(double theta_deg) {
  theta_deg = std::abs(theta_deg);
  while (theta_deg > 90) theta_deg = std::abs(180 - theta_deg);
  const auto i = static_cast<size_t>(ceil(theta_deg / 10));
  if (i == 0) return 1;
  const auto a = DIR_COEFF_A_D[i - 1];
  const auto b = DIR_COEFF_B_D[i - 1];
  const auto c = DIR_COEFF_C_D[i - 1];
  const auto d = DIR_COEFF_D_D[i - 1];
