...
inline Matrix4x4 Translation(Vector3 v, Quaternion q) {
...
inline Quaternion AngleAxis(Float a, const Vector3& v) { return Quaternion::AngleAxis(a, v); }
inline Matrix4x4 Translation(const Vector3& v, const Quaternion& q) { return Matrix4x4::Translation(v, q); }
#endif
template <typename V>
inline Vector3 ToVector3(const V& v) {
  Vector3 r;
  r(0) = v(0);
  r(1) = v(1);
  r(2) = v(2);
