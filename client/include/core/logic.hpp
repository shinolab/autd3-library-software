...
class Logic {
 public:
  ...
  [[nodiscard]] static Result<bool, std::string> WaitMsgProcessed(LinkPtr link, const uint8_t msg_id, const size_t num_devices, uint8_t* rx_data,
    ...
    for (size_t i = 0; i < max_trial; i++) {
      if (auto res = link->Read(&rx_data[0], buffer_len); res.is_err()) return res;

      size_t processed = 0;
      for (size_t dev = 0; dev < num_devices; dev++)
        if (const uint8_t proc_id = rx_data[dev * 2 + 1]; proc_id == msg_id) processed++;
  ...
  [[nodiscard]] static Result<std::vector<FirmwareInfo>, std::string> firmware_info_list(const LinkPtr link, const size_t num_devices) {
    auto concat_byte = [](const uint8_t high, const uint16_t low) { return static_cast<uint16_t>(static_cast<uint16_t>(high) << 8 | low); };

    const auto send_size = sizeof(RxGlobalHeader);
    const auto tx_buf = std::make_unique<uint8_t[]>(send_size);
    const auto rx_buf = std::make_unique<uint8_t[]>(num_devices * EC_INPUT_FRAME_SIZE);
    Result<bool, std::string> res = Ok(true);

    std::vector<uint16_t> cpu_versions(num_devices);
    res = SendHeader(link, num_devices, CMD_READ_CPU_VER_LSB, &tx_buf[0], &rx_buf[0]);
    if (res.is_err()) return Err(res.unwrap_err());
    for (size_t i = 0; i < num_devices; i++) cpu_versions[i] = rx_buf[2 * i];
    res = SendHeader(link, num_devices, CMD_READ_CPU_VER_MSB, &tx_buf[0], &rx_buf[0]);
    if (res.is_err()) return Err(res.unwrap_err());
    for (size_t i = 0; i < num_devices; i++) cpu_versions[i] = concat_byte(rx_buf[2 * i], cpu_versions[i]);

    std::vector<uint16_t> fpga_versions(num_devices);
    res = SendHeader(link, num_devices, CMD_READ_FPGA_VER_LSB, &tx_buf[0], &rx_buf[0]);
    if (res.is_err()) return Err(res.unwrap_err());
    for (size_t i = 0; i < num_devices; i++) fpga_versions[i] = rx_buf[2 * i];
    res = SendHeader(link, num_devices, CMD_READ_FPGA_VER_LSB, &tx_buf[0], &rx_buf[0]);
    if (res.is_err()) return Err(res.unwrap_err());
    for (size_t i = 0; i < num_devices; i++) fpga_versions[i] = concat_byte(rx_buf[2 * i], fpga_versions[i]);
