...
class NLS : public Holo {
 public:
  NLS(const BackendPtr& backend, const std::vector<core::Vector3>& foci, const std::vector<double>& amps, const double eps, const size_t k_max,
 ...
 public:
  ...
  LM(const BackendPtr& backend, const std::vector<core::Vector3>& foci, const std::vector<double>& amps, const double eps_1, const double eps_2,
     const double tau, const size_t k_max, std::vector<double> initial)
      : NLS(backend, foci, amps, eps_1, k_max, std::move(initial)), _eps_2(eps_2), _tau(tau) {}
 ...
 */
class GaussNewton final : public NLS {
 public:
  /**
   * @brief Generate function
   * @param[in] backend linear algebra calculation backend
   ...
   * @param[in] initial initial phase of transducers
   */
  static std::shared_ptr<GaussNewton> create(const BackendPtr& backend, const std::vector<core::Vector3>& foci, const std::vector<double>& amps,
                                             const double eps_1 = 1e-6, const double eps_2 = 1e-6, const size_t k_max = 500,
                                             const std::vector<double>& initial = {}) {
    return std::make_shared<GaussNewton>(backend, foci, amps, eps_1, eps_2, k_max, initial);
  }

  void calc(const core::GeometryPtr& geometry) override;

  GaussNewton(const BackendPtr& backend, const std::vector<core::Vector3>& foci, const std::vector<double>& amps, const double eps_1,
              const double eps_2, const size_t k_max, std::vector<double> initial)
      : NLS(backend, foci, amps, eps_1, k_max, std::move(initial)), _eps_2(eps_2) {}
 ...
 */
class GradientDescent final : public NLS {
 public:
  /**
   * @brief Generate function
   * @param[in] backend linear algebra calculation backend
   ...
   * @param[in] initial initial phase of transducers
   */
  static std::shared_ptr<GradientDescent> create(const BackendPtr& backend, const std::vector<core::Vector3>& foci, const std::vector<double>& amps,
                                                 const double eps = 1e-6, const double step = 0.5, const size_t k_max = 2000,
                                                 const std::vector<double>& initial = {}) {
    return std::make_shared<GradientDescent>(backend, foci, amps, eps, step, k_max, initial);
  }

  void calc(const core::GeometryPtr& geometry) override;

  GradientDescent(const BackendPtr& backend, const std::vector<core::Vector3>& foci, const std::vector<double>& amps, const double eps,
                  const double step, const size_t k_max, std::vector<double> initial)
      : NLS(backend, foci, amps, eps, k_max, std::move(initial)), _step(step) {}
...
class APO final : public Holo {
 public:
  ...
  static std::shared_ptr<APO> create(const BackendPtr& backend, const std::vector<core::Vector3>& foci, const std::vector<double>& amps,
    ...
    return std::make_shared<APO>(backend, foci, amps, eps, lambda, k_max);
  }

  void calc(const core::GeometryPtr& geometry) override;

  APO(const BackendPtr& backend, const std::vector<core::Vector3>& foci, const std::vector<double>& amps, const double eps, const double lambda,
      const size_t k_max)
      : Holo(backend, foci, amps), _eps(eps), _lambda(lambda), _k_max(k_max) {}
