...
static bool is_msg_processed(const size_t num_devices, const uint8_t msg_id, const uint8_t* const rx) {
  size_t processed = 0;
  const auto* rx_ = reinterpret_cast<const RxMessage*>(rx);
  for (size_t dev = 0; dev < num_devices; dev++)
    if (rx_[dev].msg_id == msg_id) processed++;
 ...
 * \return message id
 */
static uint8_t pack_header(const Modulation& mod, const uint8_t fpga_ctrl_flag, const uint8_t cpu_ctrl_flag, uint8_t* const data,
                           size_t* const mod_sent) {
  const uint8_t msg_id = get_id();
  pack_header(msg_id, fpga_ctrl_flag, cpu_ctrl_flag, data);
  if (*mod_sent == mod.buffer().size()) return msg_id;
 ...
 */
static size_t pack_body(const Gain& gain, uint8_t* data) {
  auto* header = reinterpret_cast<GlobalHeader*>(data);
 ...
 * \details This function must be called after pack_header
 */
static size_t pack_body(const PointSequence& seq, const Geometry& geometry, uint8_t* data, size_t* const seq_sent) {
  if (*seq_sent == seq.control_points().size()) return sizeof(GlobalHeader);
 ...
 * \details This function must be called after pack_header
 */
static size_t pack_body(const GainSequence& seq, const Geometry& geometry, uint8_t* data, size_t* const seq_sent) {
  if (*seq_sent == seq.gains().size() + 1) return sizeof(GlobalHeader);
    ...
    case GAIN_MODE::PHASE_HALF:
      for (const auto& dev : geometry)
        for (const auto& trans : dev) {
          ...
          const auto phase2 = static_cast<uint8_t>(gain_idx + 1 >= seq.size() ? 0x00 : seq.gains()[gain_idx + 1]->data()[trans.id()].phase & 0xF0);
          const auto phase3 =
              static_cast<uint8_t>(gain_idx + 2 >= seq.size() ? 0x00 : seq.gains()[gain_idx + 2]->data()[trans.id()].phase >> 4 & 0x0F);
          const auto phase4 = static_cast<uint8_t>(gain_idx + 3 >= seq.size() ? 0x00 : seq.gains()[gain_idx + 3]->data()[trans.id()].phase & 0xF0);
